...
const patch = { nodeReplace: [new Node({ id: 'n2', type: 'process', name: 'n2-update', inputs: n2.inputs, outputs: n2.outputs })] }

const defaultImpact = analyzeImpact(state, patch, { direction: 'downstream', includeSeeds: true })
assert.deepEqual(defaultImpact.nodes.map((n) => n.id).sort(), ['n2', 'n3'])
...
assert.deepEqual(strictImpact.nodes.map((n) => n.id).sort(), ['n2'])
